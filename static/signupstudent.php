...
function createStudent() {
    ...
    if (file_exists($xmlFile)) {
        ...
        if ($xml) {
            ...
            if (isUniqueStudent($xml, $username, $email)) {
                // Generate a new ID for the student based on the existing IDs
                $existingIds = $xml->xpath('//student/@id');
                $maxId = 0;
                ...
                $newUser->addAttribute('role', 'student');
                ...
                // Add details to the new student
                $newDetails = $newStudent->addChild('details');
                $newDetails->addChild('fullname')->addChild('fname', $firstname);
                $newDetails->fullname->addChild('lname', $lastname);
                $newDetails->addChild('birthdate', ''); // Empty or default value for date
                $newDetails->addChild('tel', 0); // 0 or another default integer value
                $newDetails->addChild('email', $email);
                $newDetails->addChild('cin', '');
                $newDetails->addChild('cne', '');
                $newDetails->addChild('apoge', 0); // 0 or another default decimal value
                $newDetails->addChild('adresse', ''); // Empty or default value for address
                $newDetails->addChild('mark', 0); // 0 or another default decimal value
                ...
                header('Location: http://localhost/SchoolManagement/static/loginstudent.php');
                exit;
            } else {
...
function isUniqueStudent($xml, $username, $email) {
    // Check if the username, email, and cin are unique among professors
    $existingUsernames = $xml->xpath('//student/@username');
    $existingEmails = $xml->xpath('//student/details/email');
                ...
                <div class="col-sm-10 col-md-8 col-lg-6 col-xl-5 mx-auto d-table h-100">
                    <div class="d-table-cell align-middle">
                        ...
                        <div class="card">
                            <div class="card-body">
                                <div class="m-sm-3">
                                    ...
                                    <form method="POST" action="signupstudent.php" enctype="multipart/form-data">
                                        <div class="mb-3">
                        ...
                        <div class="text-center mb-3">
                           <strong> Vous avez déjà un compte ? </strong><a href="loginstudent.php">Log In</a>
                        </div>
                    </div>
                </div>
            </div>
